...
async def populate_queue(workqueue: Workqueue):
    ...
    try:
        messages = await mail_service.check_inbox_messages(
            limit=100, mailbox_address="rpa.bfr@odense.dk"
        )
        ...
        for message in messages:
            if workqueue.get_item_by_reference(
                message["internet_message_id"]
            ):  # Check om mail allerede i workqueue
                logger.debug(
                    f"Message {message['internet_message_id']} already in workqueue, skipping"
                )
                continue
            ...
            # Get full email body
            body_data = await mail_service.get_message_body(
                "rpa.bfr@odense.dk", message["id"]
            )
            ...
            if body_data:
                ...
                workqueue.add_item(
                    data=workqueue_data, reference=message["internet_message_id"]
                )
        ...
        with item:
            ...
            try:
                ...
                attachment = await mail_service.get_first_file_attachment_bytes(
                    mailbox_address="rpa.bfr@odense.dk", message_id=email_id
                )
                ...
                modtager = dubu.brugere.soeg_modtager_bruger(
                    borger_sag["primaerBehandler"]["navn"],
                    str(borger_sag["primaerBehandler"]["email"]).split("@")[0],
                )

                if modtager is not None:
                ...
                behandlet_mappe = await mail_service._find_folder_by_name(
                    mailbox_address="rpa.bfr@odense.dk",
                    folder_name="Journaliseret opmærksomhedsskema",
                )

                if not behandlet_mappe:
                    raise WorkItemError(
                        "Mappen 'Journaliseret opmærksomhedsskema' blev ikke fundet i mailboksen"
                    )
...
def initialize_sync_services():
    ...
    roboa_credential = Credential.get_credential(
        "RoboA"
    )  # bruges til at logge ind på DUBU
